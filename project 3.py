...
root.iconbitmap(r'C:\Users\asus\Downloads\typing.png')
root.maxsize(600, 400)
root.minsize(600, 400)

words = [
    'banana', 'orange', 'lemon', 'pineapple', 'strawberry', 'grapefruit', 'watermelon', 'cucumber', 'tomato',
    'broccoli', 'spinach', 'potato', 'carrot', 'lettuce', 'zucchini', 'cauliflower', 'eggplant', 'asparagus',
    'bellpepper', 'celery', 'cabbage', 'radish', 'onion', 'garlic', 'mushroom', 'avocado', 'blueberry',
    'raspberry', 'blackberry', 'cherry', 'kiwi', 'apricot', 'peach', 'pear', 'plum', 'cranberry', 'pomegranate',
    'papaya', 'coconut', 'fig',
    'hair', 'sudden', 'karachi', 'for', 'today', 'that', 'yesterday', 'fork', 'door', 'color', 'floor', 'flour',
    'grape',
...
def startGame(event):
    ...
    if (wordentry.get() == word["text"]):
        matched += 1  # if you enter the correct word, then it will be counted; otherwise, the else loop will run
        random.shuffle(words)  # shuffle words in the list
        word.configure(text=words[0])  # print shuffled words one by one
    elif (wordentry.get() != word["text"]):
        not_matched += 1
        pygame.mixer.music.load("buzzer1.mp3")
        pygame.mixer.music.play(loops=0)  # it will play a sound when you enter the wrong word
    wordentry.delete(0, END)  # after the user types the word and presses the Enter key, the entry box will be automatically cleared
...
label.place(x=68, y=250)  # instruction 1 for the user below the entry label at the start of the game
...
scorelabel.place(x=50, y=310)  # text that will appear after time is up

scorecount = Label(root, text="", bg="#001D3D", fg="#FFC300", font="comicsanms 17 bold")
scorecount.place(x=190, y=310)  # number of WPM score which will be displayed between the text of scorelabel

feedback = Label(root, text="", bg="#001D3D", fg="#FFC300", font="comicsanms 12 bold")
feedback.place(x=53, y=350)  # it will display the text that tells whether your typing speed is average or not
